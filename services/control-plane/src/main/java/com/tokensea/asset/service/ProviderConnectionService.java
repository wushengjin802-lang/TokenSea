...
public class ProviderConnectionService {
    ...
    private final boolean localTestUpstreamEnabled;
    private final Set<String> localTestUpstreamHosts;
    ...
    public ProviderConnectionService(ProviderSecretMapper secrets, CryptoService crypto,
            ...
            @Value("${tokensea.local-test-upstream.enabled:false}") boolean localTestUpstreamEnabled,
            @Value("${tokensea.local-test-upstream.hosts:}") String localTestUpstreamHosts) {
        ...
        this.localTestUpstreamEnabled=localTestUpstreamEnabled;
        this.localTestUpstreamHosts=Arrays.stream(localTestUpstreamHosts.split(","))
                .map(v->v.trim().toLowerCase(Locale.ROOT)).filter(v->!v.isBlank()).collect(Collectors.toUnmodifiableSet());
        ...
        this.directClient=HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(5))
    ...
    public TestResult test(ProviderInstance instance){
        ...
        try{resolvedAddresses=resolveAddresses(target);}catch(Exception e){return TestResult.failed("PROVIDER_DNS_RESOLUTION_FAILED","供应商主机无法解析为允许的地址");}
        String apiKey;
        ...
        try{
            HttpRequest.Builder request=HttpRequest.newBuilder(requestUri(target)).GET().timeout(Duration.ofSeconds(10)).header("Accept","application/json");
            applyAuthentication(request,instance.getApiStyle(),apiKey);
            HttpResponse<InputStream> response=clientFor(target).send(request.build(),HttpResponse.BodyHandlers.ofInputStream());
            ...
            if(response.statusCode()>=200&&response.statusCode()<300)return TestResult.success(response.statusCode(),target,resolvedAddresses);
    ...
    public DiscoveryResult discoverModels(ProviderInstance instance){
        ...
        try{
            HttpRequest.Builder request=HttpRequest.newBuilder(requestUri(target)).GET().timeout(Duration.ofSeconds(20)).header("Accept","application/json");
            applyAuthentication(request,instance.getApiStyle(),apiKey);
            HttpResponse<InputStream> response=clientFor(target).send(request.build(),HttpResponse.BodyHandlers.ofInputStream());
            byte[] bytes;
    ...
    Target target(String baseValue){
        ...
        String normalizedBase=baseValue.trim().replaceAll("/+$","");
        String path=base.getPath()==null?"":base.getPath();
        String modelsEndpoint=path.endsWith("/models")?normalizedBase:normalizedBase+"/models";
        return new Target(URI.create(modelsEndpoint),host,port);
    }
    ...
    private boolean isLocalTestTarget(Target target){return localTestUpstreamEnabled&&localTestUpstreamHosts.contains(target.host());}
    private HttpClient clientFor(Target target){return isLocalTestTarget(target)?directClient:client;}
    private URI requestUri(Target target){
        if(!isLocalTestTarget(target))return target.uri();
        try{return new URI(target.uri().getScheme(),null,"127.0.0.1",target.port(),target.uri().getPath(),target.uri().getQuery(),null);}
        catch(Exception e){throw new IllegalArgumentException("本地测试上游地址无效",e);}
    }
    private String resolveAddresses(Target target) throws Exception{
        if(isLocalTestTarget(target))return "127.0.0.1";
        return resolvePublicAddresses(target.host());
    }
    private static String resolvePublicAddresses(String host) throws Exception{
        Set<String> addresses=Arrays.stream(InetAddress.getAllByName(host))
                ...
                .map(InetAddress::getHostAddress).collect(Collectors.toCollection(java.util.TreeSet::new));
        if(addresses.isEmpty())throw new IllegalArgumentException();
        return String.join(",",addresses);
    }
    record Target(URI uri,String host,int port){}
    public record TestResult(boolean success,Integer httpStatus,String errorCode,String error,String targetHost,Integer targetPort,String resolvedAddresses){
        static TestResult success(int status,Target target,String resolvedAddresses){return new TestResult(true,status,null,null,target.host(),target.port(),resolvedAddresses);}
        static TestResult failed(String code,String message){return new TestResult(false,null,code,message,null,null,null);}
    }
