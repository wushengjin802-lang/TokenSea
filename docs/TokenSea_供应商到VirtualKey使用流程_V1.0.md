...
| 租户名称 | 研发中心 |
| 租户类型 | 内部租户 |
| 可用服务模型 | deepseek-v4-pro 或 chat-pro |
| 月预算 | 10000 元 |
| 状态 | 启用 |

启用租户不会自动创建 Virtual Key。完成项目和应用配置后，必须进入 API Key 页面单独创建 Key；默认使用应用级 Key，项目级和租户级仅用于明确的高级共享场景。
...
| 所属租户 | 研发中心 | 必填 |
| Key 归属层级 | 应用级（推荐） | 默认值；项目级、租户级为高级共享场景 |
| 所属项目 | 智能客服项目 | 应用级和项目级必填 |
| 所属应用 | 客服机器人后端 | 应用级必填 |
| 允许调用的服务模型 | deepseek-v4-pro / chat-pro | 只能从租户可用服务模型中选择 |
| RPM 限制 | 60 | 每分钟请求数 |
